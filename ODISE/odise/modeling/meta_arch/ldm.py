...
def build_ldm_from_cfg(cfg_name) -> _LatentDiffusion:
    ...
    if cfg_name.startswith("v1"):
        url_prefix = "https://hf-mirror.com/CompVis/stable-diffusion/main/configs/stable-diffusion/"  # 替换为 Hugging Face 镜像地址
    elif cfg_name.startswith("v2"):
        url_prefix = "https://hf-mirror.com/Stability-AI/stablediffusion/main/configs/stable-diffusion/"  # 替换为 Hugging Face 镜像地址
...
class LatentDiffusion(nn.Module):
    ...
    LDM_CONFIGS = {
        ...
        "/home/wubinxu/peract/sd-v1-3.ckpt" : ("v1-inference.yaml", (512, 512), (64, 64)),
    }
    ...
    def __init__(
        ...
        init_checkpoint="/home/wubinxu/peract/sd-v1-3.ckpt",
    ):
...
class LdmFeatureExtractor(FeatureExtractor):
    ...
    def forward(self, batched_inputs):
        ...
        device = image.device
        self.to(device)
        pixel_mean = self.pixel_mean.to(device)
        pixel_std = self.pixel_std.to(device)
        batch_size = image.shape[0]

        if self.image_preprocess is None:
            normalized_image = (image - pixel_mean) / pixel_std
        else:
            normalized_image = self.image_preprocess((image - pixel_mean) / pixel_std)
